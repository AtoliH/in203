...
namespace Bitonic
{
  ...
  _sort( bool up, Obj* objs, int len, int depth = 0)
  {
    ...
    auto compute_first = [&first, objs, len, depth](){
        first = _sort(true, objs, len/2, depth+1);    
    };
    auto compute_second = [&second, objs, len, depth](){
        second = _sort(false, objs + len/2, len - (len/2), depth+1);
    };

    if (2 << depth > NUM_THREADS*2)
    {
        compute_first();
        compute_second();
    }
    ...
    {
        std::thread first_thread(compute_first);
        std::thread second_thread(compute_second);
